...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<tr onclick = 'copyOnClick($pcnN)'><td>".$row["model"]."</td><td>".$row["description"]."</td><td id = '$pcnID' onclick='copyOnClick($pcnN)'>".$row["pcn"]."</td><td id = '$serialNum' onclick='copyOnClick($serialN)'>".$row["serial"]."</td><td>".$row["calibration"]."</td><td>".$row["area"]."</td><td>".$row["name"]."</td><td>".$row["checkoutDate"]."</td><td>".$row["returnDate"]."</td></tr>";
		$pcnN = $pcnN+1;
        $serialN = $serialN +1;
    }
    echo "</table></div>";
